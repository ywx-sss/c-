...
string replace(const string& input, const map<string, string>& variables) {
    string result;
...
int main() {
    ...
    while (true) {
        ...
        getline(cin, input);
        if(input[0]=='?'){
        	string ans=input.substr(2);
        	cout<<ans<<"="<<variables[ans]<<endl;
		}
		else if(input=="end"){
			break;
		}
			...
			string leftstr = input.substr(0, equalpos);
        	string rightstr = input.substr(equalpos + 1);
        	rightstr = replace(rightstr,variables);
        	string postfix = getansfix(rightstr);
        	int result = getans(postfix);
			if (result == INT_MAX) {
            	cout << "Divide 0." << endl;
        	} else {
            	variables[leftstr]=to_string(result);
        	}
		}
    }
    return 0;
